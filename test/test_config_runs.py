...
def test_daily_regression_with_embedding(
    get_config: Fixture[Callable[[str], dict]], single_timescale_model: Fixture[str]
):
...
def test_multi_timescale_regression(
    get_config: Fixture[Callable[[str], dict]], multi_timescale_model: Fixture[str]
):
    ...
    discharge = hourlycamelsus.load_hourly_us_netcdf(config.data_dir, config.forcings[0]).sel(
        basin=basin, date=slice(test_start_date, test_end_date)
    )["qobs_mm_per_hour"]
...
def _check_results(config: Config, basin: str, discharge: pd.Series = None):
    ...
    assert discharge.loc[test_start_date:test_end_date].values == approx(
        results[f"{config.target_variables[0]}_obs"].values.reshape(-1), nan_ok=True
    )
...
def _get_test_start_end_dates(config: Config) -> Tuple[pd.Timestamp, pd.Timestamp]:
    ...
    test_end_date = pd.to_datetime(config.test_end_date, format="%d/%m/%Y") + pd.Timedelta(
        days=1, seconds=-1
    )
