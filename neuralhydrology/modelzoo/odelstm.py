...
class ODELSTM(BaseModel):
    ...
    def _init_frequency_factors_and_slice_timesteps(self):
        for i, freq in enumerate(self._frequencies):
            ...
            if i > 0:
                ...
                if (
                    self.cfg.seq_length[freq]
                    > self.cfg.seq_length[self._frequencies[i - 1]] * prev_frequency_factor
                ):
                    raise ValueError(
                ...
                if (
                    self.cfg.predict_last_n[self._frequencies[i - 1]]
                    < self.cfg.predict_last_n[freq] / prev_frequency_factor
                ):
        ...
        if any(
            self.cfg.predict_last_n[f] / self._frequency_factors[f]
    ...
    def _prepare_inputs(
        self, data: Dict[str, torch.Tensor], freq: str
    ...
    def _randomize_freq(
        self, x_d: torch.Tensor, low_frequency: str, high_frequency: str
    ) -> torch.Tensor:
        ...
        possible_aggregate_steps = list(
            filter(lambda n: frequency_factor % n == 0, range(1, frequency_factor + 1))
        )
        ...
        while t < max_t:
            ...
            if highfreq_slice.shape[0] % random_aggregate_steps == 0:
                randfreq_slice = highfreq_slice.view(
                    -1,
                    random_aggregate_steps,
                    highfreq_slice.shape[1],
                    highfreq_slice.shape[2],
                ).mean(dim=1)
                # update the frequency indicators.
                randfreq_slice[:, :, -1] = (
                    random_aggregate_steps / self._frequency_factors[high_frequency]
                )
            else:
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        slice_one = self._randomize_freq(
            x_d[self._frequencies[0]][: -self._slice_timesteps[self._frequencies[1]]],
            self.cfg.ode_random_freq_lower_bound,
            self._frequencies[0],
        )
        ...
        for freq in self._frequencies:
            ...
            end_step = -int(
                self.cfg.predict_last_n[freq]
                * self._frequency_factors[self._frequencies[-1]]
            ...
            if (
                end_step != prev_freq_end_step
                and end_step > -self.cfg.seq_length[self._frequencies[-1]]
            ):
                slice_three = self._randomize_freq(
                    x_d[self._frequencies[-1]][prev_freq_end_step:end_step],
                    prev_freq,
                    self._frequencies[-1],
                )
    ...
    def _run_odelstm(
        self, input_slice: torch.Tensor, h_0: torch.Tensor, c_0: torch.Tensor
...
class _ODERNNCell(nn.Module):
    ...
    def _euler(
        self, inputs: torch.Tensor, hidden_state: torch.Tensor, delta_t: float
    ) -> torch.Tensor:
