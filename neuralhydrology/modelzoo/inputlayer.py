...
class InputLayer(nn.Module):
    ...
    def __init__(self, cfg: Config, embedding_type: str = "full_model"):
        ...
        self.dynamics_embedding, self.dynamics_output_size = self._get_embedding_net(
    ...
    def forward(
        self, data: Dict[str, torch.Tensor], concatenate_output: bool = True
        ...
        if not concatenate_output:
            ...
            if self._num_autoregression_inputs:
                ret_val = torch.cat(
                    [ret_val, x_d[:, :, -self._num_autoregression_inputs :]], dim=-1
                )
