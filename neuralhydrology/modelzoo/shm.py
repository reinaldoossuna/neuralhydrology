...
class SHM(BaseConceptualModel):
    ...
    def forward(
        ...
        pwp = (
            torch.tensor(0.8, dtype=torch.float32, device=x_conceptual.device) * parameters["sumax"]
        )

        # Storages
        ss = torch.tensor(
            self.initial_states["ss"], dtype=torch.float32, device=x_conceptual.device
        ).repeat(x_conceptual.shape[0])
        sf = torch.tensor(
            self.initial_states["sf"], dtype=torch.float32, device=x_conceptual.device
        ).repeat(x_conceptual.shape[0])
        su = torch.tensor(
            self.initial_states["su"], dtype=torch.float32, device=x_conceptual.device
        ).repeat(x_conceptual.shape[0])
        si = torch.tensor(
            self.initial_states["si"], dtype=torch.float32, device=x_conceptual.device
        ).repeat(x_conceptual.shape[0])
        sb = torch.tensor(
            self.initial_states["sb"], dtype=torch.float32, device=x_conceptual.device
        ).repeat(x_conceptual.shape[0])
