...
class HandoffForecastLSTM(BaseModel):
    ...
    def __init__(self, cfg: Config):
        ...
        if not cfg.state_handoff_network:
            raise ValueError(
                "The handoff forecast LSTM requires a state handoff network specified in the config file."
            )
    ...
    def _reset_parameters(self):
        ...
        if self.cfg.initial_forget_bias is not None:
            self.hindcast_lstm.bias_hh_l0.data[
                self.hindcast_hidden_size : 2 * self.hindcast_hidden_size
            ] = self.cfg.initial_forget_bias
            self.forecast_lstm.bias_hh_l0.data[
                self.forecast_hidden_size : 2 * self.forecast_hidden_size
            ] = self.cfg.initial_forget_bias
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if x_h.shape[0] > self.initial_hindcast_seq_length:
            ...
            pred = {
                key: torch.cat([y_hindcast[key], y_hindcast_overlap[key], y_forecast[key]], dim=1)
                for key in y_hindcast
            }
        else:
