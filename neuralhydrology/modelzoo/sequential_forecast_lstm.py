...
class SequentialForecastLSTM(BaseModel):
    ...
    def __init__(self, cfg: Config):
        ...
        if self.forecast_embedding_net.output_size != self.hindcast_embedding_net.output_size:
            raise ValueError(
                "Forecast and hindcast embedding nets must have the same output size when using a sequential forecast LSTM."
            )
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        lstm_output_forecast, (h_n_forecast, c_n_forecast) = self.lstm(
            x_f, (h_n_hindcast, c_n_hindcast)
        )
