...
class ARLSTM(BaseModel):
    ...
    def __init__(self, cfg: Config):
        ...
        if self.output_size != self._num_ar_inputs:
            raise ValueError(
                "The AR-LSTM currently only works if all outputs are used as lagged inputs."
            )
    ...
    def _get_ar_shift(self) -> int:
        ...
        for input in self.cfg.autoregressive_inputs:
            ...
            if not capture[1] in self.cfg.target_variables:
                raise ValueError(
                    "Autoregressive inputs must be a shifted target variable. ",
                    f"Instead got a shifted version of: {capture[1]}.",
                )
            shifts.add(int(capture[2]))
        if len(shifts) > 1:
            raise ValueError(
                "Only one AR shift is allowed currently. All autoregressive inputs must use the same shift."
            )
        shift = shifts.pop()
        if shift <= 0:
    ...
    def _reset_parameters(self):
        ...
        if self.cfg.initial_forget_bias is not None:
            self.cell.bias_hh_l0.data[self.cfg.hidden_size : 2 * self.cfg.hidden_size] = (
