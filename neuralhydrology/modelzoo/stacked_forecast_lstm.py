...
class StackedForecastLSTM(BaseModel):
    ...
    def __init__(self, cfg: Config):
        ...
        self.forecast_lstm = nn.LSTM(
            input_size=forecast_input_size, hidden_size=self.forecast_hidden_size
        )
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        forecast_inputs = torch.concat(
            (x_f, lstm_output_hindcast), dim=-1
        )  # concat along variable dimension
