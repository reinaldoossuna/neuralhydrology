            ...
            for freq in frequencies:
                ...
                if freq not in ensemble_sum[basin]:
                    ...
                    for target_var in target_vars:
                        ensemble_sum[basin][freq][f"{target_var}_sim"] += freq_results[
                            f"{target_var}_sim"
                        ]
    ...
    for basin in tqdm(ensemble_sum.keys()):
        for freq in frequencies:
            ...
            freq_date_range = pd.date_range(
                ...
                end=ensemble_xr.coords["date"].values[-1] + pd.Timedelta(days=1, seconds=-1),
                freq=freq,
            )
            ...
            for target_var in target_vars:
                # average predictions
                ensemble_xr[f"{target_var}_sim"] = ensemble_xr[f"{target_var}_sim"] / len(
                    results_files
                )
...
def _get_medians(results: dict, metric="NSE") -> dict:
    ...
    for freq in frequencies:
        if len(frequencies) > 1:
            key = f"{metric}_{freq}"
        metric_values = [
            v[freq][key] for v in results.values() if freq in v.keys() and key in v[freq].keys()
        ]
...
def _get_best_validation_runs(run_dirs: List[Path], k: int, epoch: int = None) -> List[Path]:
    ...
    val_files = list(
        zip(
            run_dirs,
            [_get_results_file(run_dir, "validation", epoch) for run_dir in run_dirs],
        )
    )
...
def _get_results_file(run_dir: Path, period: str = "test", epoch: int = None) -> Path:
    ...
    if epoch is not None:
        dir_results_files = list(
            Path(run_dir).glob(f"{period}/model_epoch{str(epoch).zfill(3)}/{period}_results.p")
        )
    else:
...
def _main():
    ...
    parser.add_argument(
        "--period", type=str, choices=["train", "validation", "test"], default="test"
    )
    parser.add_argument(
        "--output-dir", type=str, help="Path to directory, where results are stored."
    )
    parser.add_argument(
        "--metrics",
        type=str,
