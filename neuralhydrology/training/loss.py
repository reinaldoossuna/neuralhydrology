    ...
    def __init__(
        ...
        if cfg.target_loss_weights is None:
            weights = torch.tensor(
                [1 / len(cfg.target_variables) for _ in range(len(cfg.target_variables))]
            )
        else:
...
class MaskedRMSELoss(BaseLoss):
    ...
    def _get_loss(
        ...
        **kwargs,
        ...
        mask = ~torch.isnan(ground_truth["y"])
        loss = torch.sqrt(
            0.5 * torch.mean((prediction["y_hat"][mask] - ground_truth["y"][mask]) ** 2)
        )
        return loss
...
class MaskedGMMLoss(BaseLoss):
    ...
    def __init__(self, cfg: Config, eps: float = 1e-10):
        super(MaskedGMMLoss, self).__init__(
            cfg,
...
class MaskedUMALLoss(BaseLoss):
    ...
    def __init__(self, cfg, eps: float = 1e-5):
        super(MaskedUMALLoss, self).__init__(
            cfg,
