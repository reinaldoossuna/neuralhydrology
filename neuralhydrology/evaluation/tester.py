...
from neuralhydrology.evaluation.utils import (
...
class BaseTester(object):
    ...
    def evaluate(
        ...
        for basin in pbar:
            ...
            for freq in ds.frequencies:
                ...
                feature_scaler = (
                    self.scaler["xarray_feature_scale"][self.cfg.target_variables].to_array().values
                )
                feature_center = (
                ...
                if metrics:
                    for target_variable in self.cfg.target_variables:
                        ...
                        if not all(obs.isnull()):
                            ...
                            var_metrics = (
                                metrics if isinstance(metrics, list) else metrics[target_variable]
                            )
                            if "all" in var_metrics:
                                var_metrics = get_available_metrics()
                            try:
                                values = calculate_metrics(
                                    obs, sim, metrics=var_metrics, resolution=freq
                                )
                            ...
                            if len(self.cfg.target_variables) > 1:
                                values = {
                                    f"{target_variable}_{key}": val for key, val in values.items()
                                }
    ...
    def _create_and_log_figures(self, results: dict, experiment_logger: Logger, epoch: int):
        ...
        for target_var in self.cfg.target_variables:
            max_figures = min(
                self.cfg.validate_n_random_basins, self.cfg.log_n_figures, len(basins)
            )
    ...
    def _save_results(
        self, results: Optional[dict], states: Optional[dict] = None, epoch: int = None
    ):
            ...
            for data in loader:
                ...
                for freq in frequencies:
                    ...
                    y_hat_sub, y_sub = self._subset_targets(
                        model, data, predictions, predict_last_n[freq], freq_key
                    )
        ...
        if len(losses) == 0:
            ...
            for loss_name in losses[0].keys():
                ...
                mean_losses[loss_name] = (
                    np.nanmean(loss_values) if not np.all(np.isnan(loss_values)) else np.nan
                )
