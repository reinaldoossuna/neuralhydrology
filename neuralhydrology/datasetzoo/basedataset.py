        ...
        if (self.cfg.train_data_file is None) or (not self.is_train):
            ...
            for basin in tqdm(self.basins, disable=self._disable_pbar, file=sys.stdout):
                ...
                for (
                    holdout_variable,
                    holdout_dict,
                ) in self.cfg.random_holdout_from_dynamic_features.items():
                ...
                for i, (start_date, end_date) in enumerate(zip(start_dates, end_dates)):
                    ...
                    if not all(
                        to_offset(freq).is_on_offset(start_date) for freq in self.frequencies
                    ):
                        misaligned = [
                            freq
                            for freq in self.frequencies
    ...
    def _create_lookup_table(self, xr: xarray.Dataset):
        ...
        for basin in tqdm(basin_coordinates, file=sys.stdout, disable=self._disable_pbar):
            ...
            if self.cfg.autoregressive_inputs:
                for freq in self.frequencies:
                    x_d[freq] = np.concatenate(
                        [
                            x_d[freq],
                            df_resampled[self.cfg.autoregressive_inputs].values,
                        ],
                        axis=1,
                    )
            ...
            if valid_samples.size > 0:
                if self.cfg.forecast_inputs:
                    ...
                    self._x_h[basin] = {
                        freq: torch.from_numpy(_x_d[:, hindcast_indexes].astype(np.float32))
                        for freq, _x_d in x_d.items()
                    }
                    self._x_f[basin] = {
                        freq: torch.from_numpy(_x_d[:, forecast_indexes].astype(np.float32))
                        for freq, _x_d in x_d.items()
                    }
                    ...
                    self._x_d[basin] = {
                        freq: torch.from_numpy(_x_d[:, conceptual_indexes].astype(np.float32))
                        for freq, _x_d in x_d.items()
                    }
                    self._x_d_c[basin] = {
                ...
                self._y[basin] = {
                    freq: torch.from_numpy(_y.astype(np.float32)) for freq, _y in y.items()
                }
                if x_s:
                    self._x_s[basin] = {
            ...
            for key, val in feature_specs.items():
                ...
                if key == "centering":
                    if (val is None) or (val.lower() == "none"):
                        ...
                        self.scaler["xarray_feature_center"][feature] = xr[feature].median(
                            skipna=True
                        )
                    elif val.lower() == "min":
