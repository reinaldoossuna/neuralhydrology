...
def preprocess_camels_cl_dataset(data_dir: Path):
    ...
    files = [
        f for f in list(data_dir.glob("*day.csv")) if any([x in f.name for x in available_features])
    ]
    ...
    for basin in tqdm(
        basins, file=sys.stdout, desc="Creating per-basin dataframes and saving to disk"
    ):
    ...
    print(
        f"Finished processing the CAMELS CL data set. Resulting per-basin csv files have been stored at {dst_dir}"
    )
