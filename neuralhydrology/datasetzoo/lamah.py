...
class LamaH(BaseDataset):
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        ...
        if self.cfg.use_frequencies:
            if any(
                [utils.compare_frequencies(freq, "1D") == 1 for freq in self.cfg.use_frequencies]
            ):
                temporal_resolution = "1H"

        df = load_lamah_forcing(
        ...
        if any([f.startswith("qobs") for f in self._all_variables]):
            ...
            discharge = load_lamah_discharge(
                data_dir=self.cfg.data_dir,
                basin=basin,
                temporal_resolution=temporal_resolution,
                normalize_discharge=False,
            )
            df["qobs"] = _normalize_discharge(
                ser=discharge["qobs"],
    ...
    def _load_attributes(self) -> pd.DataFrame:
        ...
        return load_lamah_attributes(
            self.cfg.data_dir, sub_dataset=self.cfg.dataset, basins=self.basins
        )


def load_lamah_forcing(
    data_dir: Path, basin: str, sub_dataset: str, temporal_resolution: str = "1D"
) -> pd.DataFrame:
    ...
    forcing_dir = (
        data_dir
        / _SUBDATASET_TO_DIRECTORY[sub_dataset]
        / "2_timeseries"
        / temporal_resolution_directory
    )
    return _load_lamah_timeseries_csv_file(forcing_dir / f"ID_{basin}.csv", temporal_resolution)


def load_lamah_discharge(
    data_dir: Path,
    basin: str,
...
def _load_lamah_timeseries_csv_file(filepath: Path, temporal_resolution: str) -> pd.DataFrame:
    ...
    df["date"] = pd.to_datetime(
        df.YYYY.map(str) + "/" + df.MM.map(str) + "/" + df.DD.map(str),
        format="%Y/%m/%d",
    )
